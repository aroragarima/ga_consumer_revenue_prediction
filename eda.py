...
def target_variable_exploration(df):
    ...
    plt.scatter(
        range(gdf.shape[0]), np.sort(np.log1p(gdf["totals.transactionRevenue"].values))
    )
...
def extract_plots(df, top, l):
    ...
    for col in l:
        name = col.replace(".", " ")
        print(name)
        cnt_srs = df.groupby(col)["totals.transactionRevenue"].agg(
            ["size", "count", "mean"]
        )
        ...
        plot_horizontally(
            cnt_srs.iloc[:, 1], cnt_srs.index, "{} -Non Zero Revenue Count".format(name)
        )
        plot_horizontally(
            cnt_srs.iloc[:, 2], cnt_srs.index, "{} -Mean Revenue".format(name)
        )
